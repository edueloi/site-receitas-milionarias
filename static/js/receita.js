...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_BASE_URL = (isLocalHost && runningLiveServer)
    ? 'http://localhost:3001'      // Dev: front 5500 -> back 3001
  ...
  async function fetchSingleRecipe() {
    ...
    if (!recipeId) {
      ...
      recipeSummaryEl.textContent = 'ID da receita não fornecido. Volte para a lista e selecione uma receita.';
      document.querySelector('.recipe-detail-page').innerHTML = '<p class="error-message container">ID da receita não fornecido. Volte para a lista e selecione uma receita.</p>';
      return;
    }
    ...
    try {
      ...
      if (breadcrumbsEl) {
        const truncatedTitle = recipe.titulo.length > 30 ? recipe.titulo.substring(0, 30) + '...' : recipe.titulo;
      ...
      if (heroEl) {
        ...
        heroEl.style.backgroundImage = `url('${buildImageUrl(recipe.imagem_url)}')`;
      }
      ...
      if (creatorEl) creatorEl.innerHTML = `<i class="fas fa-user"></i><span><strong>Criador:</strong> ${recipe.criador?.nome ?? 'Equipe'}</span>`;
      ...
      recipeSummaryEl.textContent = 'Não foi possível encontrar a receita solicitada. Por favor, tente novamente.';
    }
  }
